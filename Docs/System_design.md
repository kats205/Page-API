...
flowchart TD
    ...
    DLQ["Kafka topic: dead_letter<br/>Prometheus theo dõi offset<br/>Alertmanager bắn Slack"]
...
- **Dead Letter Queue (Kafka Topic `dead_letter`):** Lưu vĩnh viễn các message quá số lần retry. Có tích hợp Prometheus theo dõi offset và Alertmanager bắn cảnh báo (Slack/Email) cho Admin. Admin dùng Kafdrop để review.
